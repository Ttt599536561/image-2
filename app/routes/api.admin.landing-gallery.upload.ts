// POST /api/admin/landing-gallery/upload（F-073 首页画廊图片本地上传，multipart 单文件 `file`）。
// 双守卫之一：requireAdmin（每请求查 DB role + 未封禁）。落 landing/<yyyy>/<mm>/<uuid> → 返回公有 URL。
...
import { putLandingImage } from "../../src/server/r2.server";
import type { Route } from "./+types/api.admin.landing-gallery.upload";
...
export async function action({ request }: Route.ActionArgs) {
  try {
    ...
    const { publicUrl } = await putLandingImage({ bytes, contentType: mime, ext: UPLOAD_EXT[mime] });
    return Response.json({ imageUrl: publicUrl });
  } catch (e) {
    if (e instanceof Response) return e;
    console.error("[api.admin.landing-gallery.upload] error", e);
