...
日期：2026-07-12
状态：本文范围已在 `0.2.0` 实现，并于 2026-07-13 在腾讯云生产环境完成引导部署。`v0.2.2` 修复合法请求的 jq 流式聚合校验和后台已受理等待状态。
...
`v0.2.1` 已成为 stable/latest Release，但生产首次请求因 `jq --stream` 未聚合事件而在认领前被错误拒绝。修复、测试和一次性恢复设计见 [2026-07-14 热修复设计](2026-07-14-system-updater-validation-hotfix-design.md)；实际发布与生产证据见 [PROGRESS.md](../../PROGRESS.md)，命令边界见 [deploy.md](../../dev/deploy.md)。
...
`requestedBy` 只用于审计关联，不参与宿主机授权。宿主机忽略其他字段并拒绝未知键、重复请求、过大文件、非普通文件和协议版本不匹配。重复键检查使用 `jq --stream --slurp` 聚合全部流事件后统计顶层键，不能逐事件计算数组长度。
...
页面在执行中每两秒轮询。Web 重启造成的请求失败显示“服务重启中”，不能误报更新失败；新 Web 上线后读取宿主机状态并显示新版本。浏览器从启动响应中保留请求 ID；宿主仍为 `idle` 或尚未发布同一请求时显示“更新请求已提交，等待主机更新器接收”，不能静默禁用按钮。若超过健康检查上限仍无法连接，它只显示“无法确认结果”和固定兜底命令 `sudo /usr/local/sbin/ai-image-workshop-update status REQUEST_ID`，不得自行判断更新已失败。
