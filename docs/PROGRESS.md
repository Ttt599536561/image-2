...
- 现在做到哪：
  - 代码线：本地 main 已完成 /welcome 公开落地页及其三轮视觉改版
    （F-070/F-071 均 ✅ 已截图验收），工作区干净，全部测试 455 过/1 跳过。
  - 发布线：生产仍跑 v0.2.0；v0.2.1（对话图二次编辑 F-014）已发布待部署；
    v0.2.2（后台更新器修复 F-052）为发布候选；落地页改动**尚未构建发布**。
- 下一步做什么：owner 准备新开会话提新需求。新会话开始先按 CLAUDE.md
  SOP 读本文件 + FEATURES.md。若新需求不涉及发布，落地页上线与
  v0.2.2 发布继续挂起，等 owner 发话。
- 待定拍板事项：方案 B（把根域名 / 指向落地页，目前落地页只在 /welcome）
  ——已多次建议，owner 未表态，下次可再提。
- 已知风险/坑：
