...
- [ ] **阶段二·账号+积分+存储**（**施工计划已批准并落库** → [docs/dev/PHASE2-PLAN.md](dev/PHASE2-PLAN.md)，7 阶段可勾选，基于 5 路多代理精读 02–11 综合）：① 地基(Neon 双连接/Drizzle schema+迁移+seed/R2) → ② 鉴权(Better Auth) → ③ **钱链路**(账本/批次/⓪双守卫扣费/兑换/调账/对账，命门) → ④ 管线+契约(DB-as-queue/generate* 真后台/6 值归一) → ⑤ 前端接真(loader+TanStack Query 换 mock/资产库批量删除) → ⑥ 后台(/admin/* 全套+看板) → ⑦ cron/可观测/**9 例真库测试**/成本对账上线闸。在 `main` 上开 `phase2` 分支；离线先行 ① schema/④ 契约，接真待密钥。
  ...
  - [ ] **③ 钱链路（命门）→ ⑦**：下一步。预算熔断/入队三闸/抢占状态机/⓪双守卫扣费/兑换/调账/对账 + ④ 管线 + ⑤ 前端接真 + ⑥ 后台 + ⑦ cron/9 例真库测试。putToR2 往返 + ④ 落图待 R2 密钥。**`assert-no-secrets-in-bundle.ts`(CI 密钥断言)留 ⑦**。
