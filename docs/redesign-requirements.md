...
> 状态：本文 §1-§25 的基线已于 2026-07-13 部署到腾讯云生产环境，§26 对话结果图编辑（原随 `v0.2.1` 发布）已随 `v0.2.7` 于 2026-07-25 在生产上线，当前生产版本为 `0.2.9`。证据见 [PROGRESS.md](PROGRESS.md)。本文件是当前 v2 的**完整产品规格**，后续只在出现新增需求时继续修订。
...
- **项目分组（新需求 2026-07-25，✅ 已实现，2026-07-26 随 v0.2.9 上线）**：侧栏「最近」区改为项目区，参考 codex 类工具——默认无项目；首次生图后自动懒创建「默认项目」并把该会话归入；项目行带文件夹图标，点击展开/收起会话（状态记 localStorage）；hover 项目行出现「…」（重命名项目，居中弹窗）与「+」（新建项目，居中弹窗，**新项目插入列表首部**）；会话/项目均支持长按（350ms）拖动排序——会话仅限同项目内，跨项目不可移动；搜索态平铺回退为扁平会话列表。实现：`projects` 表 + `conversations.project_id/sort_order`（迁移 0009_projects.sql，存量会话一次性归档默认项目）+ `GET/POST /api/projects`、`PUT /api/projects/order`、`PATCH /api/projects/:id`、`PUT /api/projects/:id/order` + `Sidebar.tsx` 项目区重构（Pointer Events 长按手势，`<a draggable={false}>` 防 HTML5 拖拽劫持）。
...
2. **「最近」会话列表（§10）**：按 `updated_at` 倒序、竖向列表；首屏 20 条、下滑分页（每页 20）；空态"还没有对话，点「新建生成」开始吧"+淡图标。**2026-07-26 起按项目分组（默认项目 + 懒创建 + 长按排序），详见 §10 项目分组条目**。
