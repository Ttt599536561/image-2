...
- **🅿️ 第二批需求队列（4 项·方案已过审 2026-06-22·待开发）**（详见 [PROGRESS.md](docs/PROGRESS.md)「🆕 第二批待开发需求队列」+ 规格 §9/§2.2，原型 `docs/prototypes/new-features-2026-06-22.html`）：① 广播公告**编辑/删除**（同步用户端）② 用户端公告**详情弹窗 + 看完保留**（修现状「打开即已读→看完消失」）③ 后台**顶部留白/页眉**（站长第 3 次反馈）④ **图生图**（Composer 已有占位；**前置=探测中转 `/images/edits`**，同 S6 范式，不通过则阻塞）。**先不开发，待站长拍板 PROGRESS 里 3 个决策点**；建议序 ③→①②→④a 探测→④b。
