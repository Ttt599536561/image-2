...
- **🚧 第二批需求队列（4 项·方案已过审 2026-06-22·站长 3 决策点已答、开发中）**（详见 [PROGRESS.md](docs/PROGRESS.md)「🆕 第二批待开发需求队列」+ 规格 §9/§2.2，原型 `docs/prototypes/new-features-2026-06-22.html`）：① ✅ 广播公告**编辑/删除**（同步用户端；编辑表单「重新提醒」勾选=重置已读重弹红点）② ✅ 用户端公告**详情弹窗 + 看完保留**（铃铛拉全部近 50·已读灰显·红点只计未读·点弹 Modal 不删；**连带修 `image_expiring` 删图未清理→改 `deleteExpiredImages` 同批删提醒**）—— **①②已完成 `4cdf905`** ③ ✅ 后台**顶部留白/页眉**（站长第 3 次反馈）→ **`c568008`**（真因=`.main` 引用未定义 `var(--space-7)` 致 padding 全失效；改 sticky 页眉 + 已定义令牌）④ **图生图**（Composer 已有占位；决策=同价 0.07·单张参考图·不限付费；**前置=探测中转 `/images/edits`**，同 S6 范式，不通过则阻塞）。**下一步 = ④a 探测**（③①②✅）。
