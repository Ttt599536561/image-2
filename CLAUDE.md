...
- **🅿️ 待开发需求队列**（详见 [PROGRESS.md](docs/PROGRESS.md)「待开发需求队列」+ [redesign-requirements.md](docs/redesign-requirements.md) §9/§10）：① **后台通知配置/管理**（广播公告；用户端铃铛已有、后台缺下发入口，现仅 cron 自动 `image_expiring`）—— **下一步**；② ✅ **重命名会话**（左栏「最近」铅笔行内改名 → `PATCH /api/conversations/:id` + `renameConversation` + `Sidebar` 行内编辑 + rename-smoke 7/7，**已完成**）。
