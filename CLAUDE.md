...
- **✅ 第二批需求队列（4 项·2026-06-22·全部完成）**（详见 [PROGRESS.md](docs/PROGRESS.md)「🆕 第二批待开发需求队列」+ 规格 §9/§2.2）：① ✅ 广播公告**编辑/删除**（同步用户端；编辑表单「重新提醒」勾选=重置已读重弹红点）② ✅ 用户端公告**详情弹窗 + 看完保留**（铃铛拉全部近 50·已读灰显·红点只计未读·点弹 Modal 不删；连带修 `image_expiring` 删图未清理）—— **①② `4cdf905`** ③ ✅ 后台**顶部留白/页眉** `c568008`（真因=`.main` 引用未定义 `var(--space-7)` 致 padding 全失效；改 sticky 页眉 + 已定义令牌）④ ✅ **图生图**（探测 ④a `a715b8f` 证实中转 `/images/edits` 支持 gpt-image-2；实现 ④b：`/api/uploads` 魔数嗅探+限流 → `uploads/<uid>/` → `generations.input_image_key`〔迁移 0003〕→ `callRelay` 走 edits multipart；owner-scope/成功才扣不变；参考图「用后即弃」靠孤儿 cron 回收）。**下一步 = 站长本地浏览器验收。**
