...
- **✅ 待开发需求队列（两项均已完成 2026-06-22）**（详见 [PROGRESS.md](docs/PROGRESS.md)「待开发需求队列」+ [redesign-requirements.md](docs/redesign-requirements.md) §9/§10）：① ✅ **后台通知配置/管理 = 广播公告**（`_admin/notifications` 撰写页 + `api.admin.notifications`(requireAdmin) + `broadcastAnnouncement`〔per-user 批插 `notifications`、`announcement:<aid>:<uid>` 幂等、INSERT+审计同事务〕+ `NotificationBell` 按 type 分支 + link 安全分类器 `src/lib/announcementLink`；**通知开关/参数②留后续**）；② ✅ **重命名会话**（左栏铅笔行内改名 → `PATCH /api/conversations/:id` + `renameConversation` + `Sidebar` 行内编辑）。
