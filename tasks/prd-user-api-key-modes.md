...
状态：本文全部需求已在 `0.2.0` 实现，并于 2026-07-13 部署到腾讯云生产环境。生产证据与周期性运维动作只看 [PROGRESS.md](../docs/PROGRESS.md)。
...
- relay 边界必须脱敏实际 system/custom Key。custom 失败保持 custom 模式和本地 Key，
  不自动调用 system。
- `CUSTOM_KEY_MODES_ENABLED` 缺失或 `false` 时 fail closed。Debian 安装器显式写入
  `true`，使新部署立即开放 custom；关闭时为 `503` 且零写入，UI 不得静默切
...
凭据生命周期、脱敏、移动端 Key 配置和 rollback containment。验证范围包括单元测试、
金额测试、类型检查、生产构建、秘密扫描、Compose 配置和部署契约测试；不在文档中固
定测试数量，以免新增用例后产生过期结论。

生产环境已经完成部署、迁移、容器健康、管理员入口、custom 配置和加密往返检查；
产品版本为 `0.2.0`，生产提交为 `c5131aaa0335250a3846c380519324fbbf4b231b`。
真实第三方凭据的 system/custom t2i/i2i、零本站扣费复查、备份恢复和 rollback 演练按运维周期执行，
不属于未实现需求。具体状态与命令分别见 [PROGRESS.md](../docs/PROGRESS.md) 和
[deploy.md](../docs/dev/deploy.md)。
