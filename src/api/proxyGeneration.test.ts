...
describe('generateImageViaProxy', () => {
  ...
  it('does not send any api key in the request body', async () => {
    ...
    expect(JSON.parse(init.body)).toEqual({ request: generationInput });
  });

  it('redacts proxy HTTP failure details using generic secret patterns', async () => {
