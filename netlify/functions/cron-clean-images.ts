// Scheduled Function（北京 01:00 = UTC 17:00，schedule 在 netlify.toml）：图片清理（真相源 06 §7.5 / 10 §11.7）。
// 顺序 ⓪到期前 1 天预扫写通知 → ①付费顺延兜底 → ②删过期图(先删 R2 再删 DB 行 + image_cleaned) → ③扫孤儿对象。
//
// 🔴 红线：扫描/删行走 HTTP（非钱事务）；先删 R2 再删 DB（反则留孤儿）；删失败的 key 下轮重扫；
//    通知 dedupe_key ON CONFLICT DO NOTHING 防重发；cron try/catch → alert(cron_failed) + Sentry。
